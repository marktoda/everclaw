...
src/
  ...
  agent/
    ...
    tools.ts            Tool definitions (15 tools in 4 categories)
    executor.ts         Tool executor: dispatches tool calls, path containment security
...
**Path containment.** `resolvePath` in `executor.ts` validates that all file tool paths resolve within one of three writable directories (`data/notes/`, `skills/`, `tools/`). Paths that escape are rejected.
