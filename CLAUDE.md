...
- **Channel startup is resilient**: Adapters start one at a time via `ChannelRegistry.startAll()`. If one fails (e.g., invalid Discord token), the error is logged and remaining adapters still start — one bad adapter doesn't kill the system. Telegram's bot polling crash calls `process.exit(1)`, killing all channels.
