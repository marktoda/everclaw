...
describe("orchestration integration tests", () => {
  it(
    "sleep_for: task suspends and resumes after 1 second",
    async () => {
  ...
  it(
    "sleep_until: task suspends and resumes at target time",
    async () => {
      sendMessageSpy.mockClear();
  ...
  it(
    "wait_for_event + emit_event: cross-task event coordination",
    async () => {
      sendMessageSpy.mockClear();
  ...
  it(
    "spawn_workflow: parent spawns child that completes",
    async () => {
      sendMessageSpy.mockClear();
  ...
  it(
    "send_message tool: spawns send-message task",
    async () => {
      sendMessageSpy.mockClear();
  ...
  it(
    "cancel_task: cancels a sleeping task",
    async () => {
      sendMessageSpy.mockClear();
      ...
      try {
        ...
        sendMessageSpy.mockClear();
        taskDeps.anthropic = new FakeAnthropic(
          makeCancelTaskScenario(sleepResult.taskID),
        ) as any;
        ...
        const result = await db.pool.query(
          `SELECT state FROM absurd.t_test WHERE task_id = $1`,
          [sleepResult.taskID],
        );
