...
const DIR_MAPPINGS: Array<{ prefix: string; dirKey: keyof Pick<ExecutorDeps, "notesDir" | "skillsDir" | "toolsDir">; dir: "notes" | "skills" | "scripts" }> = [
...
function resolvePath(input: string, deps: ExecutorDeps): { abs: string; dir: "notes" | "skills" | "scripts" } | null {
...
export const fileTools: ToolHandler[] = [
  {
    def: defineTool("read_file", "Read a file from a writable directory (data/notes/, skills/, scripts/).", {
  ...
  {
    def: defineTool("write_file", "Write or overwrite a file. Side effects: writes to skills/ trigger schedule sync, writes to scripts/ auto chmod +x.", {
  ...
  },
  {
    def: defineTool("list_files", "List files in a writable directory.", {
      directory: { type: "string", description: "Directory to list: 'data/notes', 'skills', or 'scripts'" },
    }, ["directory"]),
    async execute(input, deps) {
