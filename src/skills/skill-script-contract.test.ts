...
describe("search-flights.py output schema", () => {
  it("top-level keys match slc-deal-hunter documented example", () => {
  ...
  it("flight object keys match slc-deal-hunter documented example", () => {
    const scriptPath = path.join(SCRIPTS_DIR, "search-flights.py");
    const source = fs.readFileSync(scriptPath, "utf-8");
    const keys = extractScriptOutputKeys(source, ".py");
  ...
  it("metadata keys match slc-deal-hunter documented example", () => {
    const scriptPath = path.join(SCRIPTS_DIR, "search-flights.py");
    const source = fs.readFileSync(scriptPath, "utf-8");
    const keys = extractScriptOutputKeys(source, ".py");
